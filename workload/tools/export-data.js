...
A.SUBJECTS.forEach(s => {
  ...
  const staff = rows.filter(r => !r.vacancy);
  const loads = staff.filter(r => r.load > 0).map(r => r.load);
  tReq += a.totalRequired; tT += staff.length; tAsg += a.assigned;
  out.subjects.push({
    ...
    required: a.totalRequired, assigned: a.assigned, teachers: staff.length,
